...
const TypewriterText = memo(({ text, isLatest }) => {
    ...
    useEffect(() => {
        ...
        let i = 0;
        setDisplayedText("");
        const speed = Math.max(5, 20 - Math.floor(text.length / 50)); // Faster for longer text
    ...
    return (
        <ReactMarkdown
            components={{
                p: ({ node, ...props }) => <p className="m-0 text-inherit" {...props} />,
...
function ChatInterface() {
    ...
    useEffect(() => {
        // Continuous smooth scroll during typing animation
            ...
            <div className="relative z-10 flex flex-col h-full md:h-[90vh] md:w-[90vw] md:max-w-5xl md:mx-auto md:my-auto bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
                    ...
                    ${msg.sender === "user"
                                ...
                                >
                                    <div className={`prose prose-sm max-w-none break-words ${msg.sender === 'user' ? 'text-white' : 'text-gray-800'}`}>
                                        <TypewriterText 
                                            text={msg.text} 
                                            isLatest={msg.sender === 'bot' && index === messages.length - 1} 
                                        />
                                    </div>
